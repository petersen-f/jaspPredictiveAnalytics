  ...
  get_timeseries_signature_date <- function(index) {
    ret <- data.frame(
      index.num = as.double(as.numeric(as.POSIXct(index))),

        year      = as.integer(lubridate::year(index)),
        year.iso  = as.integer(lubridate::isoyear(index)),
        half      = as.integer(lubridate::semester(index)),
        quarter   = as.integer(lubridate::quarter(index)),
        month     = as.integer(lubridate::month(index)),
        month.xts = as.integer(lubridate::month(index)) - 1L,
        #month.lbl = lubridate::month(index, label = TRUE, abbr = FALSE),
        day       = as.integer(lubridate::day(index)),
        hour      = as.integer(lubridate::hour(index)),
        minute    = as.integer(lubridate::minute(index)),
        second    = as.integer(lubridate::second(index)),
        wday      = as.integer(lubridate::wday(index)),
        wday.xts  = as.integer(lubridate::wday(index)) - 1L,
        #wday.lbl  = lubridate::wday(index, label = TRUE, abbr = FALSE),
        mday      = as.integer(lubridate::mday(index)),
        qday      = as.integer(lubridate::qday(lubridate::as_date(index))),
        yday      = as.integer(lubridate::yday(index)),
        mweek     = as.integer(stringi::stri_datetime_fields(lubridate::as_date(index))$WeekOfMonth),
        week      = as.integer(lubridate::week(index)),
        week.iso  = as.integer(lubridate::isoweek(index))

      )
    ret$diff      <- as.double(c(NA, diff(ret$index.num)))
    ret$week2     <- as.integer(ret$week %% 2)
    ret$week3     <- as.integer(ret$week %% 3)
    ret$week4     <- as.integer(ret$week %% 4)
    ret$mday7     <- as.integer(ret$mday %/% 7) + 1L
    ret$hour12    = ifelse(ret$hour > 12, ret$hour - 12,ret$hour)
    ret$am.pm     = ifelse(ret$hour > 12, 2,1)
    return(ret)
  }
