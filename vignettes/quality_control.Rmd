...
measurement at the beginning of the cycle - in other words the sensor measurements for
...
accuracy. Additionally, we have two variables called ***day_new_time_since*** and
...
has been on directly relates to its temperature - which affects the size of our produced
...
series quite rapidly. The hypothetical explanation is that a part inside the machine and
broke and subsequently heats it up drastically which decreases the size of our products.
Since it is undesirable to let the process exceed the control bounds, we will test in this
tutorial whether the module can accurately predict whether and at which time point the
process goes out-of-control in the past! This is called forecast verification as we test
whether our model predictions can properly predict events in the past to make sure we can
trust our models. Afterwards we will illustrate how we can use the JASP module to predict
the unseen future.
...
window cross validation* or *historical forecast evaluation*.
...
<img src ="img/univar_tutorial_sim/forecast_eval_plan.JPG" width = "246px">

<p>Figure 5: Creating an evaluation plan</p>
...
observations. The CRPS on the other hand generalizes the MAE as it computes it for the whole
predictive distribution instead of only a single forecast value for each observation. The
R-squared value tells us how much variance is explained by the forecast.
...
[here](getting_started.html#ensemble-bayesian-model-averaging) and in more
extensively [here](background.html#ensemble-bayesian-model-averaging).
...
table that includes the eBMA model. Across all slices the prophet model receives
...
the corresponding control plot looks like this:
...
option lets us select the model that we want to use to predict the future. Here we simply
...
classical time series models and machine learning algorithms) via both point-based
